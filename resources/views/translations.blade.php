...
<div class="card bg-base-100 shadow-md">
    <div class="p-4 max-w-xl">
        <label for="translateLanguageExampleId" class="label mb-2 text-sm font-medium text-base-content">{{ __('interpresso::translations.example_language.label') }}</label>
        ...
        <p class="text-sm text-base-content/70 py-3">{{ __('interpresso::translations.example_language.info', ['language' => config('app.fallback_locale')]) }}</p>
    </div>
    ...
    @if($isAdministrator)
        <div class="flex flex-wrap justify-end p-4">
            @php($onlyModels = \AnyMedia\Interpresso\Models\Setting::getCached()->db_loader)
            @include('interpresso::partials.action-form', ['url' => route('interpresso.translations.export', ['language' => $language] + request()->query()), 'text' => __('interpresso::translations.button.' . ($onlyModels ? 'export_translation_models' : 'export_translation')), 'fields' => ['exportOnlyModels' => (int) $onlyModels]])
            @include('interpresso::partials.action-form', ['url' => route('interpresso.translations.approve-all', ['language' => $language] + request()->query()), 'text' => __('interpresso::translations.button.approve_all', ['language_code' => $language->code]), 'fields' => []])
        </div>
    @endif
