...
export async function json(url, { method = 'GET', body, signal } = {}) {
    ...
    try {
        ...
        const error = new Error(uiText('invalid_response', 'The server returned an invalid response. Please try again.'));
        error.status = response.status;
        throw error;
    }
    if (!response.ok) {
        const error = new Error(data?.message || uiText('request_failed', 'The request failed. Please try again.'));
        error.status = response.status;
        throw error;
    }
    return data;
