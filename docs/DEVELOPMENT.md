...
- The Blade `component.button` and `partials.action-form` includes accept optional `variant` (`primary`, `success`, `error`, `warning`, `info`, `ghost`, `outline`, `neutral`) and `size` (`xs`, `sm`, `md`, `lg`, `xl`). Defaults remain `primary` and `md`. Pass `xs` for row actions and `sm` for page actions. Keep full DaisyUI class names in the component mapping so Tailwind discovers them. Use semantic variants for consequences and neutral filled states for applied filters.
