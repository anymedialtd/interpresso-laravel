...
Verwenden Sie den ausdrücklichen PHP-CLI-Pfad Ihres Hosters, beispielsweise `/usr/local/bin/php83`. Das standardmässige `php` von Cron ist oft älter als die PHP-Version der Website. Ein im Browser funktionierender Vorgang kann deshalb vor dem Laravel-Start scheitern. Prüfen Sie `/usr/local/bin/php83 -v` und das Cron-Protokoll, bevor Sie Ausgaben verwerfen. Der Cron-Benutzer benötigt Schreibzugriff auf Speicher- und Exportpfade; Hintergrundprozesse sind optional. Prüfen Sie den Zeitplan und testen Sie einen Durchlauf mit demselben Programm:
...
Der Worker wird jede Minute mit `withoutOverlapping` geplant. `interpresso.schedule.worker_background` ist standardmässig `true` (`INTERPRESSO_SCHEDULE_WORKER_BACKGROUND`). Der Scheduler prüft sowohl `function_exists('proc_open')` als auch die PHP-Einstellung `disable_functions`. Falls Prozesse nicht gestartet werden können oder die Hintergrundausführung deaktiviert ist, führt ein benannter Rückruf `Artisan::call` im Vordergrund aus. Das blosse Entfernen von `runInBackground()` würde weiterhin einen Unterprozess starten. Auch Wartungsaufgaben verwenden Rückrufe, wenn `proc_open` nicht verfügbar ist. Die Cache-Sperre verfällt nach `ceil((max_time + timeout) / 60) + 1` Minuten, standardmässig drei Minuten, damit der letzte Auftrag enden kann. Ein regulärer Abschluss gibt sie früher frei. Verwenden Sie persistenten Dateicache auf einem Host oder gemeinsamen Cache auf mehreren Hosts, keinen flüchtigen Array-/Null-Cache. Diese Scheduler-Sperre verhindert überlappende Cron-Worker. Der separate `ProcessLock` in der Datenbank schützt Übersetzungsvorgänge über HTTP, CLI und Stapel hinweg; beide Sperren sind nötig. Manuelle Worker-Aufrufe sind nicht durch die Scheduler-Sperre geschützt.

Eingereihte Exporte, einschliesslich erzwungener und Modellexporte, Freigaben, das Ergänzen fehlender Übersetzungen und Importe verarbeiten höchstens `interpresso.chunk_size` Quellzeilen pro Auftrag. Danach reihen sie einen Nachfolger mit Cursor im selben Stapel ein. Der Standard ist `100`, einstellbar mit `INTERPRESSO_CHUNK_SIZE`; reduzieren Sie ihn bei kurzen Prozessgrenzen des Hosters. Fehlende Übersetzungen mit KI beachten zusätzlich `max_open_ai_missing_trans`. Nach einem Worker-Neustart geht es beim gespeicherten Cursor weiter. Eine abrupt beendete Reservierung kann ihren aktuellen Block nach Ablauf von `retry_after` wiederholen. Abgeschlossene Blöcke bleiben gespeichert. Cursor-Aufträge erlauben erneute Reservierungsversuche; ein tatsächlicher Verarbeitungsfehler lässt den Stapel sofort scheitern. Die Fortschrittsanzeige verwendet eine geschätzte Gesamtzahl, bei Dateiimporten anhand der Dateigrösse, und erreicht erst nach Abschluss 100%.

Datenbankquellen verwenden geordnete Primärschlüssel als Cursor. Dateiimporte verwenden die Position der Einträge und weisen zwischen Blöcken geänderte Quelldateien zurück. Halten Sie die Eingaben bis zum Stapelende unverändert. Importe und das Ergänzen fehlender Zeilen erhalten vorhandene Übersetzungen bei einer Wiederholung. Exporte führen Schlüssel zusammen und ersetzen Dateien atomar. PHP-/JSON-Eingaben und bestehende Exportdateien müssen weiterhin jeweils vollständig eingelesen werden. Teilen Sie aussergewöhnlich grosse Dateien auf, wenn eine einzelne Datei die Speicher- oder Prozessgrenze überschreitet. Eine unterbrochene KI-Anfrage kann erneut berechnet werden, wenn ihre Antwort noch nicht gespeichert war.

Der Worker-Standard von `96` MB lässt Reserven auf einem Host mit 128-256 MB. `INTERPRESSO_QUEUE_WORKER_MEMORY` oder `interpresso:work --memory=64` ändert die Speichergrenze zwischen Aufträgen; innerhalb eines Auftrags gilt weiterhin PHPs `memory_limit`. `interpresso:work --max-time=30` überschreibt das Zeitbudget für einen Aufruf. Kleinere Blöcke verkürzen einzelne Aufträge. `--max-time` wird zwischen Aufträgen geprüft und unterbricht keinen laufenden Block.

Cron funktioniert auch bei Hostern, die nur Intervalle von 5 oder 15 Minuten erlauben: Ändern Sie das erste Cron-Feld zu `*/5` oder `*/15`. Eingereihte und verzögerte Aufträge beginnen entsprechend später; Rückstände können mehrere Durchläufe benötigen. Jeder Block erneuert seinen `ProcessLock` in der Datenbank vor und nach der Arbeit. Setzen Sie `INTERPRESSO_PROCESS_LOCK_TTL` höher als Cron-Intervall plus Worker-Laufzeit und Planungsverzögerung. Der Standard von `1800` Sekunden bietet Reserven für 15-Minuten-Intervalle. Erhöhen Sie bei Upgrades eine gespeicherte TTL von `900` Sekunden für einen 15-Minuten-Cron. Bei gestopptem PHP sind keine Erneuerungen möglich. Abschluss, Fehler oder Abbruch gibt die eigene Stapelsperre frei.
...
Verarbeitet nur die konfigurierte Paketwarteschlange und endet, sobald sie leer ist oder das Zeit-/Auftragsbudget erreicht ist. Verbleibende Arbeit folgt beim nächsten Cron-Durchlauf. Eine leere Warteschlange oder ein regulärer Budgetstopp liefert 0, eine ungeeignete Verbindung oder ungültige Grenze 1; sonst wird der Worker-Exitcode weitergegeben. Auftragsfehler können auch bei Exitcode 0 gespeichert werden; prüfen Sie Fehlerdatensätze und Protokolle. Mit `--max-time` und `--memory` überschreiben Sie die Grenzen für einen Aufruf. Konfigurieren Sie die Standards wie unter [Cron ohne Supervisor](#cron-without-supervisor) beschrieben.
